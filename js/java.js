...
function check() {
    ...
    // VALIDA CEP
    let regex = /^[0-9-]+$/;
    if (cepValue === '' || cepValue.length !== 9 || !regex.test(cepValue)) {
        validarErro(cep, 'Informe um CEP válido');
    } else {
        validarSucesso(cep);
        sucesso += 1;
    }
    ...
    if (estadoValue === '' || estadoValue.length !== 2) {
        validarErro(estado, 'Inválido');
    } else {
        validarSucesso(estado);
        sucesso += 1;
    }

    // VALIDA RUA
    if (estadoValue === '') {
        validarErro(rua, 'Informe o logradouro');
    } else {
        validarSucesso(rua);
        sucesso += 1;
    }
    ...
    // VALIDA NUMERO
    if (numeroValue === '' || !regex.test(numeroValue)) {
        validarErro(numero, 'Informe um número válido');
    } else {
        validarSucesso(numero);
        sucesso += 1;
    }

    // VALIDA BAIRRO
    if (bairroValue === '') {
        validarErro(bairro, 'Preencha este campo!');
    } else {
        validarSucesso(bairro);
        sucesso += 1;
    }

    // VALIDA CIDADE
    if (cidadeValue === '') {
        validarErro(cidade, 'Preencha este campo!');
    } else {
        validarSucesso(cidade);
        sucesso += 1;
    }
